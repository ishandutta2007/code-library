...
// returns n! % mod without taking all the multiple factors of p into account
// that appear in the factorial
// mod = multiple of p
// O(mod) * log(n)
...
int ncr(ll n, ll r, int p, int k) {
  ...
  if (t >= k)
    return 0;
  int ans = 1LL * factmod(n, p, mod) * inverse(factmod(r, p, mod), mod) % mod *
