...
// primitive root modulo n exists iff n = 1, 2, 4 or n = p^k or 2 * p^k for some
// odd prime p
int primitive_root(int p) {
  ...
  if (n > 1)
    fact.push_back(n);
  for (int res = 2; res <= p; ++res) { // this loop will run at most (logp ^ 6)
                                       // times i.e. until a root is found
    bool ok = true;
