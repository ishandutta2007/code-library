...
int main() {
  ...
  //   cin >> MPE::coeff[i];
  // }
  // std::vector<u32> values = {521, 0, 261, 0, 551, 0, 464, 0, 696, 0, 406, 0,
  // 667, 0, 348, 0, 435, 0, 261, 0, 319, 0, 377, 0, 638, 0, 348, 406, 1};
  std::vector<u32> values = {34, 0, 7, 0, 28, 21, 1};
  n = values.size();
  std::copy(values.begin(), values.end(), MPE::coeff);

  // for (u32 i = 0; i < n; ++i) {
  //   cout << '\n' <<  MPE::coeff[i];
  // }

  std::vector<u32> values2 = {0, 7, 14, 21, 28, 35, 42};
  m = values2.size();
  std::copy(values2.begin(), values2.end(), MPE::point);
  // for (u32 i = 0; i < m; ++i) {
  //   cin >> MPE::point[i];
  //}
