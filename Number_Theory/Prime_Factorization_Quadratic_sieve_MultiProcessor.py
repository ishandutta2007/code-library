...
class Parallel_Factorizer:
    ...
    def parallel_mpqs(self, n):
        primes, primepowers, residues, logprime = (
            self.primes,
            self.primepowers,
            self.residues,
            self.logprime,
        ...
        a, b, Q, iter, req = (
            0,
            0,
            ceil_kth_root(ceil_kth_root(2 * n, 2) / M, 2),
            0,
            len(primes) / self.num_processes + self.extra,
        )
        ...
        while req > 0:
            ...
            for i in xrange(len(primepowers)):
                ...
                if R < 0:
                    R += pe
                for k in xrange(
                    M * (1 - pe) + getFirst(pe, M * (pe - 1), R), 2 * M + 1, pe
                ):
                    log_estimate[k] += lg
                if not (pe & (pe - 1)):
                    continue
            ...
            threshold, logMAX = (
                int(log(n / a, 2) - log(MAX, 2) - 0.5),
                log(MAX) / log(2) / 1.4,
            )
            for k in xrange(0, 2 * M + 1):
                if log_estimate[k] < threshold or req <= 0:
                    continue
                intermediate += 1
                _v = (a * (k - M) + b) ** 2 - n
                v = abs(_v / a)
                if log_estimate[k] < int(log(v + 1, 2) - logMAX):
                    continue
                tried += 1
                num, nonzeroP, nonzeroE = 0, array("l"), array("B")
                for j in xrange(len(primes) - 1):
                    p, Ej = primes[j], 0
                    num <<= 1
                    while v % p == 0:
                        v, Ej = v / p, Ej + 1
            ...
            if iter % 100 == 0 and print_process_wise_progress:
                ...
                print(
                    ...
                    ((time.time() - enter_time) * req) / (REQ - req),
                    "seconds\n\n................................................\n\n",
                )
    ...
    def factor_utility(self, n):
        ...
        init_primes, primes, primepowers, residues, logprime = (
            get_primes(init_B),
            array("l"),
            array("l"),
            array("l"),
