...
public:
  ...
  div_vector(div_vector const &o) { std::copy(o.st, o.en, st); }
  ...
  div_vector(div_vector &&o) : st(nullptr), en(nullptr) { swap(*this, o); }
...
template <div_vector_layout const &layout, typename T, typename Derived>
class vectorspace_mixin {
private:
...
public:
  ...
  friend Derived operator*(Derived const &a, T const &t) {
    return Derived(a) * t;
  }
  ...
  friend Derived operator*(T const &t, Derived const &a) {
    return t * Derived(a);
  }
...
template <div_vector_layout const &layout, typename T>
class dirichlet_series_values;
template <div_vector_layout const &layout, typename T>
class dirichlet_series_prefix;
template <div_vector_layout const &layout, typename T>
class dirichlet_series_binary_indexed_tree;

template <div_vector_layout const &layout, typename T>
class dirichlet_series_values
    : public div_vector<layout, T>,
      public vectorspace_mixin<layout, T, dirichlet_series_values<layout, T>> {
public:
  dirichlet_series_values() = default;

  template <typename F,
            std::enable_if_t<std::is_invocable_r_v<T, F, int64_t, int64_t>,
                             bool> = true>
  dirichlet_series_values(F f) {
    for (int i = 1; i < layout.len; i++) {
      this->st[i] =
          f(layout.get_bucket_bound(i - 1), layout.get_bucket_bound(i));
    }
  }

  template <typename U>
  explicit dirichlet_series_values(
      dirichlet_series_values<layout, U> const &o) {
    for (int i = 1; i < layout.len; i++) {
      this->st[i] = T(o.st[i]);
    }
  }

  explicit dirichlet_series_values(dirichlet_series_prefix<layout, T> &&o)
      : div_vector<layout, T>(
            static_cast<div_vector<layout, T> &&>(std::move(o))) {
    for (int i = layout.len - 1; i > 1; i--) {
      this->st[i] -= this->st[i - 1];
    }
  }
  explicit dirichlet_series_values(
      dirichlet_series_prefix<layout, T> const &o) {
    for (int i = layout.len - 1; i > 1; i--) {
...
class dirichlet_series_prefix
    : public div_vector<layout, T>,
      public vectorspace_mixin<layout, T, dirichlet_series_prefix<layout, T>> {
public:
  dirichlet_series_prefix() = default;

  template <typename F,
            std::enable_if_t<std::is_invocable_r_v<T, F, int64_t>, bool> = true>
  ...
  template <typename U>
  explicit dirichlet_series_prefix(
      dirichlet_series_prefix<layout, U> const &o) {
    for (int i = 1; i < layout.len; i++) {
      this->st[i] = T(o.st[i]);
    }
  }

  explicit dirichlet_series_prefix(dirichlet_series_values<layout, T> &&o)
      : div_vector<layout, T>(
            static_cast<div_vector<layout, T> &&>(std::move(o))) {
  ...
  explicit dirichlet_series_prefix(
      dirichlet_series_values<layout, T> const &o) {
...
private:
  ...
  template <typename F>
  void convolve_helper(dirichlet_series_prefix const &a,
                       dirichlet_series_prefix const &b, F f) {
    // We roughly want to apply this[N/z] += a_val[x] * b_val[y] for all xyz <=
    // N
    //
    ...
    for (int i = 2; i < layout.len; i++) {
      ...
      if (i <= layout.rt) {
        ...
        for (int x = 2; x <= x_max; x++) {
          ...
          int zlo_idx =
              int(x <= rt_over_i ? x * i : layout.len - (N_over_i / x));
          this->st[zlo_idx] += v;
          tot_sub += v;
        }
        this->en[-(i - 1)] -= tot_sub;
        ...
        {
          int zlo_idx =
              int(i <= rt_over_i ? i * i : layout.len - (N_over_i / i));
...
public:
  friend dirichlet_series_prefix operator*(dirichlet_series_prefix const &a,
                                           dirichlet_series_prefix const &b) {
    dirichlet_series_prefix r;
    r.st[1] = a.st[1] * b.st[1];
    r.convolve_helper(a, b, [&](int i, T cur_sum) -> T {
      return cur_sum + (a.st[i] - a.st[i - 1]) * b.st[1] +
             a.st[1] * (b.st[i] - b.st[i - 1]);
    });
    return r;
  }
  dirichlet_series_prefix &operator*=(const dirichlet_series_prefix &o) {
    return *this = *this * o;
  }
  ...
  friend dirichlet_series_prefix operator/(dirichlet_series_prefix const &a,
    ...
    r.convolve_helper(r, b, [&](int i, T cur_sum) -> T {
      return (a.st[i] - (cur_sum + r.st[1] * (b.st[i] - b.st[i - 1]))) *
                 inv_b1 +
             r.st[i - 1];
    });
    return r;
  ...
  //   B = prod 1 / (1 - a_i i^{-s})
  //
  // Note: strictly speaking, the standard Euler transform over a generating
  // function should be
  ...
  friend dirichlet_series_prefix
  euler_transform_fraction(dirichlet_series_prefix a_pref) {
    ...
    std::array<T, 6> invs{T{},       T(1),      inv(T(2)),
                          inv(T(3)), inv(T(4)), inv(T(5))};
    ...
    // In particular, we take e^a = 1 + a + a^2 / 2 + a^3 / 6 + a^4 / 24 + a^5 /
    // 120
    ...
    for (x--; x >= 2; x--) {
      for (int i = x; i < layout.len; i++) {
        r.st[i] +=
            r.st[layout.get_value_bucket(layout.get_bucket_bound(i) / x)] *
            a.st[x];
      }
    }
  ...
  friend dirichlet_series_prefix
  inverse_euler_transform_fraction(dirichlet_series_prefix a) {
    ...
    for (x = 2; layout.rt / x / x / x > 0; x++) {
      T v = a.st[x] - T(1);
      if (v == 0)
        continue; // Small optimization, good for prime counting in particular
      r.st[x] = v;
      for (int i = layout.len - 1; i > x; i--) {
        a.st[i] -=
            a.st[layout.get_value_bucket(layout.get_bucket_bound(i) / x)] * v;
      }
      a.st[x] = T(1);
    }
    ...
    // Phase 2: now we take log of the remaining thing, using just the first few
    // terms.
  ...
  friend dirichlet_series_prefix
  euler_transform_binary_indexed_tree(dirichlet_series_prefix a_pref) {
    int x = 2;
    while (x <= layout.N / x / x)
      x++;
  ...
  friend dirichlet_series_prefix
  inverse_euler_transform_binary_indexed_tree(dirichlet_series_prefix a_pref) {
...
// TODO: This will be useful for sparse convolution, which is nice for e.g.
// exp/log/prime counting
template <div_vector_layout const &layout, typename T>
class dirichlet_series_binary_indexed_tree
    : public div_vector<layout, T>,
      public vectorspace_mixin<
          layout, T, dirichlet_series_binary_indexed_tree<layout, T>> {
public:
  dirichlet_series_binary_indexed_tree() = default;

  template <typename U>
  explicit dirichlet_series_binary_indexed_tree(
      dirichlet_series_binary_indexed_tree<layout, U> const &o) {
    for (int i = 1; i < layout.len; i++) {
      this->st[i] = T(o.st[i]);
    }
  }
  ...
  explicit dirichlet_series_binary_indexed_tree(
      dirichlet_series_prefix<layout, T> &&o)
      : div_vector<layout, T>(
            static_cast<div_vector<layout, T> &&>(std::move(o))) {
    for (int i = layout.len - 1; i >= 1; i--) {
  ...
  explicit operator dirichlet_series_prefix<layout, T>() && {
    ...
    swap(static_cast<div_vector<layout, T> &>(r),
         static_cast<div_vector<layout, T> &>(*this));
    for (int i = 1; i < layout.len; i++) {
      r.st[i] += r.st[i & (i - 1)];
    }
    return r;
  ...
  void sparse_mul_at_most_one(int64_t x, T w) {
    assert(x > 1);
    int64_t j = 1;
    T cur = get_bucket_prefix(
        int(j <= layout.rt / x ? layout.len - j * x : layout.N / x / j));
    for (; (j + 1) <= layout.N / x / (j + 1); j++) {
      T nxt = get_bucket_prefix(int(j + 1 <= layout.rt / x
                                        ? layout.len - (j + 1) * x
                                        : layout.N / x / (j + 1)));
    ...
    for (int64_t i = layout.N / x / j; i > 0; i--) {
      ...
      increment_bucket_suffix(
          int(i <= layout.rt / x ? i * x : layout.len - layout.N / x / i),
          w * (cur - nxt));
      cur = nxt;
    }
  }
