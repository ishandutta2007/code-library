...
struct Mint {
  ...
  static uint64_t reduce(const __uint128_t &x) {
    uint64_t y = uint64_t(x >> 64) -
                 uint64_t((__uint128_t(uint64_t(x) * inv) * mod) >> 64);
...
bool suspect(const uint64_t &a, const uint64_t &s, uint64_t d,
             const uint64_t &n) {
  if (Mint::mod != n)
    Mint::set_mod(n);
...
bool is_prime(const uint64_t &n) {
  ...
  while (~d & 1)
    s++, d >>= 1;
  static const uint64_t a_big[] = {2,      325,     9375,      28178,
...
template <typename T> vector<T> factor(T n) {
  if (n < 2)
    return {};
  if (is_prime(n))
    return {n};
