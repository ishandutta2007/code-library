...
-- Kth term of Linear Recurrence, 2913ms
(https://judge.yosupo.jp/submission/85727)
- N = 50'000:
...
The main goal of this library is to implement common polynomial functionality in
a
reasonable from competitive programming POV complexity, while also doing it in
as
straight-forward way as possible.

Therefore, primary purpose of the library is educational and most of
constant-time
...
polynomial operations constitute intended solution. However, it is recommended
to
seek out other implementations when the time limit is tight or you really want
to
...
template <int m> struct modular {
      ...
      while (true) {
        ...
        struct lin {
          ...
          lin operator*(const lin &t) {
            return {a * t.a + b * t.b * y, a * t.b + b * t.a};
          }
        } x(z, 1); // z + x
        x = bpow(x, (m - 1) / 2);
        if (x.b != modular(0)) {
  ...
  constexpr modular(int64_t rr) : r(rr % m) {
    if (r < 0)
      r += m;
  }
...
template <int T> ostream &operator<<(ostream &out, modular<T> const &x) {
  return out << x.r;
}
...
template <int m> void mul(vector<modular<m>> &a, vector<modular<m>> b) {
...
template <typename T> struct poly {
  ...
  pair<poly, poly>
  divmod_hint(poly const &b, poly const &binv) const { // when inverse is known
    assert(!b.is_zero());
    if (deg() < b.deg()) {
      return {poly{0}, *this};
    }
    ...
    poly D = (reverse().mod_xk(d + 1) * binv.mod_xk(d + 1))
                 .mod_xk(d + 1)
                 .reverse(d + 1);
    return {D, *this - D * b};
  }
  ...
  divmod(const poly &b) const { // returns quotiend and remainder of a mod b
    ...
    poly D = (reverse().mod_xk(d + 1) * b.reverse().inv(d + 1))
                 .mod_xk(d + 1)
                 .reverse(d + 1);
    return {D, *this - D * b};
  }
  ...
  struct transform {
    poly a, b, c, d;
    transform(poly a, poly b = T(1), poly c = T(1), poly d = T(0))
        : a(a), b(b), c(c), d(d) {}

    transform operator*(transform const &t) {
      return {a * t.a + b * t.c, a * t.b + b * t.d, c * t.a + d * t.c,
              c * t.b + d * t.d};
    }
  ...
  poly pow(int64_t k, size_t n) {
    ...
    if (i > 0) {
      return k >= int64_t(n + i - 1) / i
                 ? poly(T(0))
                 : div_xk(i).pow(k, n - i * k).mul_xk(i * k);
    }
  ...
  vector<T> eval(vector<poly> &tree, int v, auto l,
                 auto r) { // auxiliary evaluation function
    if (r - l == 1) {
      return {eval(*l)};
    } else {
      auto m = l + (r - l) / 2;
  ...
  poly inter(vector<poly> &tree, int v, auto l, auto r, auto ly,
             auto ry) { // auxiliary interpolation function
    if (r - l == 1) {
      return {*ly / a[0]};
    } else {
      auto m = l + (r - l) / 2;
  ...
  static poly
  build(vector<poly> &res, int v, auto L,
        auto R) { // builds evaluation tree for (x-a1)(x-a2)...(x-an)
    if (R - L == 1) {
  ...
  poly shift(T a) const { // P(x + a)
    return (expx(deg() + 1).mulx(a).reverse() * invborel())
        .div_xk(deg())
        .borel();
  }
  ...
  poly inv(int n) const { // get inverse series mod x^n
    ...
    return (poly(P0f * TTf).x2() + poly(P1f * TTf).x2().mul_xk(1)).mod_xk(n);
  }
  ...
  static poly compose_large(poly A, poly B, int n) {
    ...
    function<poly(poly const &, int, int)> compose_dac =
        [&getpow, &compose_dac](poly const &f, int m, int N) {
          if (f.deg() <= 0) {
            return f;
          }
          int k = m / 2;
    ...
    while (r >= 0) {
      ans += (AB0.mod_xk(n - r * q) * rfact<T>(r) * B1p[r])
                 .mul_xk(r * q)
                 .mod_xk(n);
      r--;
      if (r >= 0) {
