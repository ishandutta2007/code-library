...
vector<ll> CLUT() {
  vector<ll> CLUTarr;
  // fast "pop count" Counting Look Up Table...
...
ll countPageFromTo(ll bitstrt, ll bitlmt, ll sb) {
  const fst = bitstrt >> 5;
  const lst = bitlmt >> 5;
  const pg = new Uint32Array(sb.buffer);
  let v0 = (pg[fst] | ((0xFFFFFFFF >>> 0) << (bitstrt & 31))) >>> 0;
  let cnt = ((lst - fst) << 5) + CLUT[v0 & 0xFFFF];
  cnt += CLUTarr[v0 >>> 16];
  for (let i = fst | 0; i < lst; ++i) {
    let v = pg[i] >>> 0;
    cnt -= CLUTarr[v & 0xFFFF];
    cnt -= CLUTarr[v >>> 16];
  }
  let v1 = (pg[lst] | ((0xFFFFFFFE >>> 0) << (bitlmt & 31))) >>> 0;
  cnt -= CLUTarr[v1 & 0xFFFF];
  cnt -= CLUTarr[v1 >>> 16];
  return cnt | 0;
}

void partialSievePage(ll lwi, ll bp, ll sb) {
  const btsz = sb.length << 3;
  let s = Math.trunc((bp * bp - 3) / 2); // compute the start index...
  if (s >= lwi)
    s -= lwi; // adjust start index based on page lower limit...
...
ll partialSieveCountPage(lwi, bp, cntarr, sb) {
  const btsz = sb.length << 3;
  let cullcnt = 0;
  let s = Math.trunc((bp * bp - 3) / 2); // compute the start index...
...
const WHLPTRNLENarr = new Uint8Array(len);
const WHLPTRN() { // larger than WHLPTRN by one buffer for overflow
  const len = (WHLPTRNLEN + (PGSZBITS >>> 3) + 3) & (-4); // up 2 even 32 bits!
  for (let bp of WHLPRMS.slice(1))
    partialSievePage(0, bp, arr);
  WHLPTRNLENarr[0] |= ~(-2 << ((WHLPRMS[WHLPRMS.length - 1] - 3) >> 1)) >>> 0;
  return WHLPTRNLENarr;
}

void fillPage(lwi, sb) {
  const mod = (lwi / 8) % WHLPTRNLEN;
  sb.set(new Uint8Array(WHLPTRN.buffer, mod, sb.length));
}

ll cullPage(ll lwi, ll bpras, ll sb) {
  const btsz = sb.length << 3;
  let bp = 3;
...
void soePages(bitsz, bpras) {
  const buf = new Uint8Array(bitsz >> 3);
  let lowi = 0;
  const gen = bpras == = undefined ? makeBasePrimeRepArrs() : bpras;
  return function * () {
    while (true) {
      fillPage(lowi, buf);
      cullPage(lowi, gen, buf);
      yield{lwi : lowi, sb : buf};
      lowi += bitsz;
    }
  };
}

void makeBasePrimeRepArrs() {
  const buf = new Uint8Array(128);
  let gen = undefined; // avoid data race!
  fillPage(0, buf);
  ...
  function next() {
    ...
    nxtpg.bpra[0] += lastgap;
    lastgap = nxtpg.lastgap;
...
vector<int> TinyPrimes{2, 3, 5, 7, 11, 13, 17, 19}; // degree eight
...
ll TinyTotientsarr = new Uint32Array(TinyLength | 0);
vector<ll> TinyTotients() {
  arr[TinyLength - 1] = 1; // mark mid point value as not prime - never is
  let spn = 3 * 5 * 7;
  arr[0] = 1;              // mark zeroth value as not prime!
  for (let bp of[3, 5, 7]) // cull small base prime values...
    for (let c = (bp + 1) >>> 1; c <= spn; c += bp)
      arr[c] |= 1;
...
ll *countPrimesTo(limit) {
  ...
  if (limit <= 32 * 2 * PGSZBITS + 3) { // count sieve to about 32 million...
    ...
    for (let pg of soePages(PGSZBITS, bpras)()) {
      ...
      if (nxti > lmti) {
        cnt += countPageFromTo(0, lmti - pg.lwi, pg.sb);
        break;
      }
  ...
  const bsprms = function() {
    let bp = 3;
    let cnt = WHLPRMS.length + 1;
    for (let bpra of bpras())
      for (let bpr of bpra) {
        bp += bpr + bpr;
        if (bp > cbrt)
          return new Uint32Array(cnt);
        ++cnt;
      }
  }
  ();
  ...
  const pis = function() { // starts with index 0!
    const arr = new Uint32Array(cbrt + 2);
    let j = 0;
    for (let i = 1; i < bsprms.length; ++i)
      for (; j < bsprms[i];)
        arr[j++] = (i - 1) >>> 0;
    for (; j < arr.length;)
      arr[j++] = (bsprms.length - 1) >>> 0;
    return arr;
  }
  ();
  const phis = function() { // index below TinyPhi degree never used...
    ...
    for (let i = WHLPRMS.length + 2; i < arr.length; ++i) {
      arr[i] -= i - WHLPRMS.length - 1;
    } // account for non phi primes!
    return arr;
  }
  ();
  ...
  const S1 = function() { // it is very easy to calculate S1 recursively...
    ...
    function level(lpfni, lmtlpfni, mfv, m) {
      while (lpfni < lmtlpfni) {
        const pn = bsprms[lpfni];
        const nm = m * pn;
        if (nm > cbrt) { // don't split, found S2 root leaf...
          specialroots[m] = (lmtlpfni << 1) | (mfv < 0 ? 1 : 0);
          return;
  ...
  function *complex() {
    ...
    for (let lwi = 0; lwi <= lwilmt; lwi += PGSZBITS) { // for all pages
      ...
      const maxbpi =
          pis[Math.min(cbrt, Math.trunc(Math.sqrt(limit / low)))] >>> 0;
      const tminbpi =
          pis[Math.min(Math.trunc(top / (high + 2)), bsprms[maxbpi]) >>> 0];
      const minbpi = Math.max(TinyPrimes.length, tminbpi) + 1;
      fillPage(lwi, buf);
      let bpi = (WHLPRMS.length + 1) >>> 0;
      ...
      if (minbpi <= maxbpi) { // jump to doing P2 if not range
        ...
        if (!brkchkr)
          for (; bpi <= maxbpi; ++bpi) {
            const bp = bsprms[bpi];
            let l =
                pis[Math.min(Math.trunc(limit / (bp * low)), cbrt) >>> 0] >>> 0;
            if (bp >= bsprms[l])
              break;
            const piminm =
                pis[Math.max(Math.trunc(limit / (bp * (high + 2))), bp) >>>
                    0] >>>
                0;
            for (; l > piminm; --l) {
      ...
      for (; bpi <= bpilmt; ++bpi)
        partialSievePage(lwi, bsprms[bpi], buf);
      let ndx = 0 >>> 0;
      while (p2v >= low && p2v < high + 2) {
        const nndx = (p2v - low) >>> 1;
        ++p2cnt;
        ttlcnt += countPageFromTo(ndx, nndx, buf);
        p2acc += ttlcnt;
        ndx = (nndx + 1) >>> 0;
        p2v = Math.trunc(limit / revgen.next().value);
      }
      if (ndx < PGSZBITS)
        ttlcnt += countPageFromTo(ndx, PGSZBITS - 1, buf);
    }
    // adjust for now known delta picbrt to pisqrt!
    p2acc -=
        p2cnt * (p2cnt - 1) / 2 + p2cnt * (bsprms.length - 1 - WHLPRMS.length);
    const Piy = bsprms.length - 1;
    return S1 + s2acc - p2acc + Piy - 1;
  }
  const gen = complex();
  let lst = gen.next();
  for (; !lst.done; lst = gen.next())
    yield lst.value;
  return lst.value;
}
...
ll doit() {
  ...
  function pgfnc() {
    if (cancelled) {
      ...
      document.getElementById('output').innerText =
          "Found " + pgrslt.value + " primes up to " + limit + " in " + elpsd +
          " milliseconds.";
    }
    cancelled = false;
