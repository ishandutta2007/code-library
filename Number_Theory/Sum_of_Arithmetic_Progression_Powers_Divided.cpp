...
vector<vector<long long>> solve(long long k1, long long k2, long long a,
                                long long b, long long c, long long n) {
  if (n < 0) {
    vector<vector<long long>> ret(k1 + k2 + 1,
                                  vector<long long>(k1 + k2 + 1, 0));
    return ret;
  }
  if (!a) {
    vector<vector<long long>> ret(k1 + k2 + 1,
  ...
  if (a >= c) {
    ...
    for (int _k1 = 0; _k1 <= k1 + k2; ++_k1) {
      ...
      for (int _k2 = 1; _k1 + _k2 <= k1 + k2; ++_k2) {
        ...
        for (int y = 0; y <= _k2; ++y) {
          ret[_k1][_k2] +=
              ((ncr[_k2][y] * power(a / c, y)) % mod) * nxt[_k1 + y][_k2 - y];
          ret[_k1][_k2] %= mod;
        }
      }
    }
    ...
    for (int _k1 = 0; _k1 <= k1 + k2; ++_k1) {
      ...
      for (int _k2 = 1; _k1 + _k2 <= k1 + k2; ++_k2) {
        ...
        for (int y = 0; y <= _k2; ++y) {
          ret[_k1][_k2] +=
              ((ncr[_k2][y] * power(b / c, y)) % mod) * nxt[_k1][_k2 - y];
          ret[_k1][_k2] %= mod;
        }
      }
    }
