...
// };
// enum EBUCKET {
//  UINT_PIMAX = 203280221,
//  MAX_BUCKET = 5465 * 2,
//  WHEEL_SIZE = 1 << 12,
//  MEM_WHEEL = WHEEL_SIZE * sizeof(int) * 2,
//  MEM_BLOCK = (1 << 19) / WHEEL_SIZE,
//  MAX_STOCK = UINT_PIMAX / WHEEL_SIZE + MAX_BUCKET + MEM_BLOCK,
//  MAX_POOL = UINT_PIMAX / (MEM_BLOCK * WHEEL_SIZE) + 100
// };
// enum EFLAG {
//  PRINT_TIME = 1 << ('T' - 'A'),
//  SLOW_TEST = 1 << ('A' - 'A'),
...
//   (32 << 10) / ERAT_SMALL,
//   0,
//   (L2_DCACHE_SIZE << 10) / ERAT_MEDIUM,
//   0,
//   SIEVE_SIZE *(WHEEL30 << 10) / ERAT_BIG,
//   100000000,
// };
// static struct Config_ Config = {
//   SIEVE_SIZE << 10,(1 << 6) - 1,PRINT_RET | PRINT_TIME,
//   ERAT_SMALL,   ERAT_MEDIUM,ERAT_BIG,
//   1 << 3,
// };
...
//   for (uint offset = p; offset < sizeof(PreSieved) * WHEEL30; offset += p *
//   2)
...
// #ifdef _WIN32
//  FILETIME ptime[4] = {0};
//  GetThreadTimes(GetCurrentThread(),&ptime[0],&ptime[1],&ptime[2],
//          &ptime[3]);
...
// #if 0
//   SievePrime *pSprime = (SievePrime*)_aligned_malloc(MEM_BLOCK *
//   MEM_WHEEL,MEM_WHEEL); WheelPool[BucketInfo.PoolSize ++] = pSprime;
// #else
//  SievePrime *pSprime = (SievePrime *)malloc((MEM_BLOCK + 1) * MEM_WHEEL);
//  WheelPool[BucketInfo.PoolSize++] = pSprime;
//  pSprime =
//    (SievePrime *)((size_t)pSprime + MEM_WHEEL - (size_t)pSprime % MEM_WHEEL);
// #endif
...
// static int setBucketInfo(const uint sieve_size,const uint sqrtp,
//              const uint64 range) {

//  BucketInfo.MaxBucket = range / sieve_size + 1;
//  BucketInfo.LoopSize = sqrtp / (sieve_size / MAX_WHEEL_GAP) + 2;
...
//  for (uint p = SmallPrime[FIRST_INDEX].Sp,j = FIRST_INDEX; p < maxp;
//     p = SmallPrime[++j].Sp) {
//   const uint64 p2 = (uint64)p * p;
//   if (p2 > offset && p2 > offset + segsize)
...
//  }
// }
// static int segmentedSieve2(uchar bitarray[],const uint start,
//               const uint sieve_size,bool bcopy);
// static void initMediumWheel(uchar *bitarray,const uint sieve_size,
//               const uint medium,const uint64 start) {
//  uint j = Threshold.L1Index,
//     l1_maxp = Threshold.L1Maxp - Threshold.L1Maxp % WHEEL210;
//  Threshold.L2Index = 0;
...
//   uint64 offset = start;
//   const uint64 p2 = (uint64)p * p;
//   if (EUNLIKELY(p2 > offset && p2 >= offset + segsize))
//    offset += (p2 - offset) / segsize * segsize;
//   uint sieve_index = p - (uint)(offset % p);
...
//    const WheelFirst &wf =
//      WHEEL_FIRST[(sieve_index + (uint)(offset % WHEEL)) % WHEEL][pi];
//    sieve_index += wf.Correct * p;
...
//  MediumPrime[j + 0].Sp = MediumPrime[j + 1].Sp = UINT_MAX;
// }
// inline static void pushBucket(const uint offset,const uint sp,
//                const ushort wi) {
...
// #ifndef P_B
//   if (EUNLIKELY(BucketInfo.CurStock-- == 0))
//    allocWheelBlock(1);
// #else
//   BucketInfo.CurStock--;
// #endif

//   Stock *psfree = StockHead;
//   StockHead = psfree->Next;
...
// }
// static void initBigWheel(uchar *bitarray,uint medium,uint sqrtp,
//              const uint64 start,const uint64 range) {
//  uint nextp = 0;
//  uint64 remp = 0;
//  if (ELIKELY(sqrtp < UINT_MAX))
//   sqrtp++;
//  const uint irange =
//    (uint)((range >> 32) > WHEEL30 ? UINT_MAX : range / WHEEL30);
//  for (uint l2_size = L2_DCACHE_SIZE * WHEEL30 << 10; medium < sqrtp;
//     medium += l2_size) {
...
// #endif
// #ifndef P_C
//    if (EUNLIKELY(sieve_index > range))
//     continue;
// #endif
//    const uint sp =
//      (p / WHEEL210 << SP_BIT) + WheelInit210[p % WHEEL210].WheelIndex;
...
//    else {
//     sieve_index = (sieve_index / WHEEL210 + (sp >> SP_BIT) * wf->Correct) *
//            (WHEEL210 / WHEEL30);
...
//    pSprime->Sp = sp;
//    pSprime->Si =
//      (sieve_index & BucketInfo.ModuloSize) << SI_BIT | wf->WheelIndex;
// #endif
//   }
//  }
// }
// static int crossSmall0(uchar bitarray[],const uchar *pend,const uint p,
//             uint offset,uint multiples) {
//  uchar *ppbeg[8],wi;
//  for (int i = 0; i < 8; i++) {
//   wi = WheelInit30[offset % WHEEL30].WheelIndex;
//   ppbeg[wi] = bitarray + offset / WHEEL30;
//   offset += (multiples % 8) * p;
//   multiples /= 8;
//  }
...
// }
// static int crossSmall1(uchar bitarray[],const uchar *pend,const uint p,
//             uint offset,uint multiples) {
//  int mini = 1 << 31;
//  for (int i = 0; i < 4; i++) {
...
// #define POSET(s1,o1,b1,s2,o2,b2,s3,o3,b3,s4,o4,b4,s5,o5,b5,s6,\
//        o6,b6,s7,o7,b7)                       \
//  while (ps <= pend) {                             \
...
//   break;
//  case 4:
//   POSET(6,3,3,8,4,4,14,7,7,18,10,1,20,11,2,24,13,5,26,14,
//      6)
//   break;
//  case 5:
//   POSET(4,2,4,10,6,2,12,7,5,18,11,3,22,13,7,24,15,1,28,17,
//      6)
//   break;
//  case 6:
//   POSET(2,1,4,6,4,5,12,9,1,14,10,6,20,15,2,24,18,3,26,19,
//      7)
//   break;
//  case 7:
//   POSET(2,1,7,8,7,6,12,11,5,14,13,4,18,17,3,20,19,2,24,23,
//      1)
//   break;
//  }
...
// static void crossMedium1(uchar bitarray[],const uint sieve_byte,
//              MediumSieve *pSprime) {
//  uint &si = pSprime->Si;
...
//  WheelElement wd[8];
//  memcpy(wd,WheelData30[pSprime->Sp % (1 << SI_BIT)],sizeof(wd));
//  WheelElement we;
//  we.WheelIndex = si % (1 << SI_BIT);
//  while ((int)offset < 0) {
//   MEDIUM_SET();
//  }
...
// static void crossMedium2(uchar bitarray[],uint sieve_byte,uint maxsp,
//              MediumSieve *pSprime) {

//  WheelElement wheelData30[8][8];
//  memcpy(wheelData30,WheelData30,sizeof(wheelData30));
...
// #endif
//   WheelElement we1;
//   we1.WheelIndex = si1 % (1 << SI_BIT);
//   WheelElement we2;
//   we2.WheelIndex = si2 % (1 << SI_BIT);
...
// }
// static void crossMedium4(uchar bitarray[],uint sieve_byte,uint maxsp,
//              MediumSieve *pSprime) {
//  WheelElement wheelData[PWS][PWS];
//  memcpy(wheelData,WheelData210,sizeof(WHEEL_DATA));
//  while (pSprime[1].Sp < maxsp) {
//   uint &si1 = pSprime[0].Si,sp1 = pSprime[0].Sp;
//   uint &si2 = pSprime[1].Si,sp2 = pSprime[1].Sp;
//   pSprime += 2;
...
// #if 1
//   WheelElement *wd2 = wheelData[sp2 % (1 << SI_BIT)];
//   WheelElement *wd1 = wheelData[sp1 % (1 << SI_BIT)];
// #else
//   WheelElement wd1[48];
//   WheelElement wd2[48];
...
// static MediumSieve *crossMediumW30(uchar bitarray[],const uint sieve_byte,
//                   const uint minsp,MediumSieve *pSprime) {

//  uchar pattern30[64];
//  memcpy(pattern30,Pattern30,sizeof(pattern30));
//  WheelElement wheelData30[8][8];
//  memcpy(wheelData30,WheelData30,sizeof(wheelData30));
...
// static inline int crossBig1(uchar bitarray[],const uint sieve_size,
//               const SievePrime *pSprime) {
...
// static int crossBig2(uchar bitarray[],const uint sieve_size,
//            const SievePrime *pSprime,
//            const WheelElement wheelData210[PWS][PWS]) {
...
//  return 2;
// }
// static void preSieve(uchar bitarray[],const uint64 start,
//            const uint sieve_size) {
//  const uint offset = (uint)(start % PRIME_PRODUCT) / WHEEL30;
//  const uint bits =
//    sieve_size / WHEEL30 * 8 + WheelInit30[sieve_size % WHEEL30].WheelIndex;
...
//  uint multiples = offset % 16 * 3;
//  offset >>= 4;
//  multiples =
//    (NEXT_MULTIPLE >> multiples) | (NEXT_MULTIPLE << (24 - multiples));
...
// static void eratSieveLeve1(uchar bitarray[],const uint64 start,
//               const uint sieve_size,uint maxp) {
...
//     (NEXT_MULTIPLE >> multiples) | (NEXT_MULTIPLE << (24 - multiples));
//   offset +=
//     crossSmall0(bitarray,pend,p,offset,multiples) + 30 * p - sieve_size;
...
// static void eratSieveLeve2(uchar bitarray[],const uint64 start,
//               const uint sieve_size,uint maxp) {
//  if (start + sieve_size < ((uint64)maxp) * maxp)
//   maxp = isqrt(start + sieve_size) + 1;
//  const uchar *pend = bitarray + sieve_size / WHEEL30;
//  for (uint p = MediumPrime[Threshold.L1Index].Sp,j = Threshold.L1Index;
//     p < maxp; p = MediumPrime[++j].Sp) {
//   uint &offset = MediumPrime[j].Si;
// #ifdef SM0
//   uint multiples = offset % 16 * 3,mask = multiples;
//   offset >>= 4;
//   multiples =
//     (NEXT_MULTIPLE >> multiples) | (NEXT_MULTIPLE << (24 - multiples));
//   offset +=
...
// static void eratSieveMedium1(uchar bitarray[],const uint64 start,
//                const uint sieve_size,const uint spi,uint maxp) {
//  if (start + sieve_size < (uint64)maxp * maxp)
//   maxp = isqrt(start + sieve_size) + 1;
//  MediumSieve *pSprime = MediumPrime + spi;
//  const uint sieve_byte = sieve_size / WHEEL30 + sieve_size % WHEEL30;
//  const uint maxsp =
//    (maxp / WHEEL30 << SI_BIT) + WheelInit30[maxp % WHEEL30].WheelIndex;
...
//  if (start + sieve_size < (uint64)maxp * maxp)
//   maxp = isqrt(start + sieve_size) + 1;
//  const uint sieve_byte = sieve_size / WHEEL30 + (sieve_size % WHEEL30 ? 1 :
//  0); const uint maxsp =
//    (maxp / WHEEL << SI_BIT) + WHEEL_INIT[maxp % WHEEL].WheelIndex;
...
//   Stock *pnext = pStock->Next;
//   pStock->Next = StockHead;
//   StockHead = pStock;
//   pStock = pnext;
//   SievePrime *pSprime = StockHead->Sprime;
//   while (loops--) {
//    pSprime += crossBig1(bitarray,sieve_size,pSprime);
//   }
//   BucketInfo.CurStock++;
//  }
...
// }
// static int segmentProcessed(uchar bitarray[],const uint64 start,uint bytes,
//               PrimeCall *pcall) {
...
//     continue;
//    }
//    const uint64 p = start + bi * WHEEL30 * sizeof(mask) + PRIME_OFFSET(mask);
//    mask &= mask - 1;
//    pcall->primes += 1;
...
// }
// static void eratSieveSmall(uchar bitarray[],const uint64 start,
//               const uint sieve_size,const uint l1_maxp) {
//  for (uint offset = 0,l1_size = Threshold.L1Size * WHEEL30;
//     offset < sieve_size; offset += l1_size) {
//   if (EUNLIKELY(l1_size + offset > sieve_size))
//    l1_size = sieve_size - offset;
//   eratSieveLeve1(bitarray + offset / WHEEL30,start + offset,l1_size,
//           l1_maxp);
//  }
// }

// static int segmentedSieve(uchar bitarray[],const uint64 start,
//              const uint sieve_size) {
...
//   c = 0;
//  }
//  for (uint offset = 0,l2_size = Threshold.L2Size * WHEEL30;
//     offset < sieve_size; offset += l2_size) {
//   if (EUNLIKELY(l2_size + offset > sieve_size))
//    l2_size = sieve_size - offset;
//   eratSieveSmall(bitarray + offset / WHEEL30,start + offset,l2_size,
//           l1_maxp);
// #ifdef SL2
//   eratSieveLeve2(bitarray + offset / WHEEL30,start + offset,l2_size,
//           l2_maxp);
// #else
//   eratSieveMedium1(bitarray + offset / WHEEL30,start + offset,l2_size,
//            Threshold.L1Index,l2_maxp);
// #endif
//  }
//  if (medium2 > l2_maxp)
// #if M210
...
//      (7 + WheelInit30[sieve_size % WHEEL30].WheelIndex) / 8;
// }
// static int segmentedSieve2(uchar bitarray[],const uint start,
//               const uint sieve_size,bool bcopy) {
...
//     offset1 = 0 - offset1;
// #endif
//    uint mask = multiples = offset1 % 16 * 3;
//    offset1 >>= 4;
//    multiples =
//      (NEXT_MULTIPLE >> multiples) | (NEXT_MULTIPLE << (24 - multiples));
//    offset1 +=
//      crossSmall0(pstart,pend,p,offset1,multiples) + 30 * p - l1_size;
//    offset = offset * 16 + mask;
//   }
//  }
// #endif

//  for (uint j = Threshold.L1Index,p = l1_maxp; p <= sqrtp;
//     p = SmallPrime[++j].Sp) {
//   uint &offset = SmallPrime[j].Si;
//   if ((int)offset <= 0) {
//    offset = p - start % p;
...
//   }
//   const WheelFirst &wf =
//     WheelFirst30[offset % WHEEL30][WheelInit30[p % WHEEL30].WheelIndex];
//   const uint multiples = (NEXT_MULTIPLE >> (wf.Multiple * 3)) |
//               (NEXT_MULTIPLE << (24 - wf.Multiple * 3));
//   offset = crossSmall1(bitarray,bitarray + sieve_size / WHEEL30,p,
//              offset + wf.Correct * p,multiples);
//  }
...
//   setL1Index();
//  } else if (level == 2 && cache >= (Threshold.L1Size >> 10) &&
//        cache <= MAX_SEGMENT) {
...
//   sieve_size = (sieve_size / l2_Size) * l2_Size << 10;
...
// #endif
//  for (uint si = 0,sieve_size = Config.SieveSize * WHEEL30; start < end;
//     start += sieve_size) {
//   if (EUNLIKELY(sieve_size > end - start))
//    sieve_size = (uint)(end - start);
//   const uint bytes = segmentedSieve(bitarray,start,sieve_size);
//   if (EUNLIKELY(align210 > 0)) {
//    memset(bitarray,~0,align210 / WHEEL30);
//    bitarray[align210 / WHEEL30] |=
//      (1 << WheelInit30[align210 % WHEEL30].WheelIndex) - 1;
//    align210 = 0;
//   }
...
//     printf(">> %.2f%%,time ~= %.2f %s,primes ~= %llu\r",tratio,timeuse,
//         "sec",picount);
//    else
//     printf(">> %.2f%%,time ~= %.3f %s,primes ~= %llu\r",tratio,
//         timeuse / 60,"min",picount);
//    fflush(stdout);
//   }
// #endif
//  }
...
//   if (r % powr == 0 && logr > 4)
//    sprintf(buff,"%de%d+%de%d",(int)(n / pown),logn,(int)(r / powr),
//        logr);
//   else if (r % powr == 0)
...
// }
// static uint setBucketStart(const uint64 start,const uint sqrtp,
//               const uint sieve_size) {
...
//   sieve_size = setSieveSize(SIEVE_SIZE);
//  if ((sieve_size & (sieve_size - 1)) != 0 &&
//    sqrtp > (sieve_size * WHEEL30) / Config.Msegs)
...
//   MediumPrime[Threshold.L1Index + 0].Sp =
//     MediumPrime[Threshold.L1Index + 1].Sp = UINT_MAX;
//   MediumPrime[pix + PI_65536 - 2].Sp = MediumPrime[pix + PI_65536 - 1].Sp =
//     UINT_MAX;
//  }
// }
...
//   initMediumWheel(bitarray,sieve_byte * WHEEL30,medium,
...
//    printf(" (%.2f + %.2f = %.2f sec %u kb L%u%u%u %d)",(ti - ts) / 1000.0,
//        (ta - ti) / 1000.0,(ta - ts) / 1000.0,Config.SieveSize >> 10,
...
//    *l1size = cache_size;
//   printf("\n cache_level %d,cache_type = %d cache_size = %d kb",cache_level,
...
//  }
//  cpuidInfo(regs,1,0);
//  int cpuCores = (regs[1] >> 16) & 0xff;
//  int l2cache[4];
//  cpuidInfo(l2cache,0x80000006,0);
//  int l3Size = 0,l1Size = 64,l2Size = ((uint)l2cache[2]) >> 16;
//  if (strstr(vendor,"AMD")) {
//   l3Size = ((uint)l2cache[3] >> 18) * 512;
//   cpuidInfo(regs,0x80000005,0);
//   l1Size = (uint)regs[2] >> 24;
...
//       llog10,rlog10,ilog(maxrange,10));
...
//   setCacheSegs(1,rand() % 5 + 1),setCacheSegs(2,rand() % 5 + 2),
//     setCacheSegs(3,rand() % 8 + 2);
//   setCacheSegs(4,rand() % 8 + 1);
//   primes += doSieve(lowerBound,end - 1,NULL);
//   if (lowerBound % 4 == 0)
//    printf("chunk: %.2f%%\r",
//        100 - (int64)(upperBound - lowerBound) * 100.0 / range),
//      fflush(stdout);
//   lowerBound = end;
//  }
//  if (lowerBound + 1 == 0)
...
//    664579, 5761455, 50847534,455052511,4118054813u,203280221,
//    155428406,143482916,133235063,124350420,116578809, 109726486,
//    103626726,98169972,0,
//  };
//  int64 ts = getTime();
...
//   setCacheSegs(1,rand() % 6 + 2),setCacheSegs(2,rand() % 6 + 2);
//   setCacheSegs(3,rand() % 6 + 2);
...
//  int64 ts = getTime();
//  for (uint l2_size = sieve_size * WHEEL30,beg = start; beg < end;
//     beg += l2_size) {
//   if (l2_size > end - beg)
//    l2_size = end - beg;
...
//    break;
//   case 'C':
//    setCacheSize(cdata % 10,cdata / 10);
//    break;
//   case 'L':
//    setCacheSegs(cdata % 10,cdata / 10);
//    break;
//   case 'M':
...
//  for (int i = 30; i > 0; i--)
//   for (int j = 1; j < 10; j++) {
//    setCacheSize(1,32 * (rand() % 1 + 1));
//    setCacheSize(2,256 * (rand() % 2 + 1));
//    uint64 beg =
//      ((uint64)(rand() * rand()) << i) + (uint64)rand() * rand() * rand();
...
//    setCacheSegs(2,rand() % 6 + 2),setCacheSegs(1,rand() % 5 + 1);
//    setCacheSegs(4,rand() % 8 + 1);
...
//   char cmd[100] = {0};
//   setCacheSize(1,32 * (rand() % 1 + 1));
//   setCacheSize(2,L2_DCACHE_SIZE * (rand() % 2 + 1));
//   setCacheSegs(1,rand() % 6 + 1);
//   setCacheSegs(2,rand() % 6 + 1);
//   setCacheSegs(3,rand() % 12 + 2);
//   uint64 beg =
//     (uint64)(rand() * rand()) * (uint64)(rand() * rand()) % mpow(10,12);
...
//   sprintf(cmd,"%llu %llu s%u L%u1 L%u2 L%u3 c%d2",beg,range,
//       Config.SieveSize >> 10,Config.L1Segs,Config.L2Segs,Config.Msegs,
//       Threshold.L2Size >> 10);
...
//        beg,range,Config.SieveSize >> 10,Config.L1Segs,Config.L2Segs,
//        Config.Msegs,Threshold.L2Size >> 10);
//    puts(cmd);
//   }
// #endif
//   if (j % 10 == 0)
...
//    setCacheSegs(3,i);
//    setCacheSegs(2,rand() % 6 + 2),setCacheSegs(1,rand() % 6 + 1);
//    const uint64 r1 = doSieve(start,end,NULL);
//    char cmd2[100] = {0};
//    sprintf(cmd2," %llu %llu s%u L%u1 L%u2 L%u3",start,end,
//        Config.SieveSize >> 10,Config.L1Segs,Config.L2Segs,
//        Config.Msegs);
//    setSieveSize(L2_DCACHE_SIZE << ((rand() % 5) + 0));
//    setCacheSegs(3,rand() % 10 + 2);
//    setCacheSegs(2,rand() % 6 + 2),setCacheSegs(1,rand() % 5 + 1);
//    const uint64 r2 = doSieve(start,end,NULL);
//    if (r1 != r2) {
//     printf("  %llu != %llu,%llu %llu s%u L%u1 L%u2 L%u3\n %s\n",r1,r2,
//         start,end,Config.SieveSize >> 10,Config.L1Segs,Config.L2Segs,
//         Config.Msegs,cmd2);
//    }
//   }
//   if (k % 10 == 0)
//    printf("\rmedium progress ~= %d %d%%\n",k,100 * k / 1000),
//      fflush(stdout);
//  }
// #endif
// #endif
