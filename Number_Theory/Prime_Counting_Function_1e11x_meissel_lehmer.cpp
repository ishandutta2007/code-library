...
const int N = 1911570 + 7, M = 632454 + 7, K = 10, P = 2 * 3 * 5 * 7 * 11 * 13,
          Q = 6, R = 29;
...
inline int get_id(ll n, ll m) { return n <= sqrt_n ? id1[n] : id2[m / n]; }
...
ll phi(ll n, ll m, ll k) {
  if (n <= P && m <= K)
    return pre_phi[m][n];
  if (m <= Q)
    return n / product[m] * pre_phi[m][product[m]] + pre_phi[m][n % product[m]];
  if (n < prime[m] * prime[m])
    return pi(n, k) - m + 1;
  if (n < prime[m] * prime[m] * prime[m])
    return pi(n, k) + p2(n, m, k) - m + 1;
