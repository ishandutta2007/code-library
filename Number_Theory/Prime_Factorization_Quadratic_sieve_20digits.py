...
def siqs_trial_division(
    n, sieve_array, factor_base, smooth_relations, g, h, m, req_relations
):
    ...
    while not success:
        ...
        while not enough_relations:
            ...
            if (
                ...
                or i_poly % 8 == 0
                and len(smooth_relations) > prev_cnt
            ):
                print(
                    "Total %d/%d relations."
                    % (len(smooth_relations), required_relations)
...
def find_prime_factors(n):
    ...
    if perfect_power:
        ...
        if digits <= MAX_DIGITS_POLLARD:
            print(
                "Using Pollard rho (Brent's variant) to factorise %d (%d digits)..."
                % (n, digits)
            )
            factors = [pollard_brent_find_factor(n)]
        else:
            print(
                "Using Self-Initializing Quadratic Sieve to factorise"
                + " %d (%d digits)..." % (n, digits)
            )
...
def factorise(n):
    ...
    if rem != 1:
        ...
        if digits > MAX_DIGITS_POLLARD:
            print(
                "Attempting quick Pollard rho (Brent's variant) to find slightly "
                + "larger factors..."
            )
