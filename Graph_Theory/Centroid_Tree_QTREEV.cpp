...
int get_centroid(int u, int size, int p = -1) {
  for (auto k : grafo[u]) {
    if (k != p && !rm[k]) {
      if (sub[k] * 2 > size) { // if there is a subtree with size greater than
                               // size/2 i can find a centroid there;
...
int pai[maxn];
int root = -1;
int decomposition(int u = 0) { // doesn't matter which beggining node,
                               // calculates the centroid decomposition of the
                               // tree;
