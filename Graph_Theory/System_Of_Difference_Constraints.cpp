...
all the given inequalities are satisfiable or not. If satisfiable, then output a
solution.
Solution:
* For each variable we create a vertex.
* For each inequality, x_i-x_j <= w, we give a directed edge (v_i, v_j ) with
cost -w.
* Create a source vertex S and give an edge (S, v_i) for all vertices with cost
0.
...
int32_t main() {
  ...
  for (int i = 1; i <= s; i++)
    d[i] = -1e9 -
           9; // 1e9 + 9 for T = 0,  try to make INF as close to 0 as possible
  d[s] = 0;
  if (!spfa(s))
    puts("-1");
  else {
