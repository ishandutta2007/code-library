...
template <typename T> struct heap {
  ...
  heap meld(heap b) { return heap(meld(root, b.root)); }
  heap insert(T x, int from,
              int to) { // from and to is needed only for this problem
    return heap(meld(root, new node(x, from, to)));
  }
  heap pop() { return heap(meld(root->ch[0], root->ch[1])); }
...
vector<ll> Eppstein(int n, int src, int dest,
                    int k) { // 0 indexed, directed graph
  vector<ll> d(n, INF);
  vector<int> par(n, -1);
  vector<int> idx(n, -1);
