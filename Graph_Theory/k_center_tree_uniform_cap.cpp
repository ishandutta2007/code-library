...
template <class weight_type> struct tree {
  ...
  Output :
          A tuple (minimum radius, set of centers opened, assignment of each
  vertex to some center)

  Algorithm Description :
          First get a sorted list of distances in O(n ^2 log(n))
          Binary search on the index in this list. This works as the answer is
  going to be distance between some pair.

          To find a feasible assignment for a given radius, we use the algorithm
  in the paper : Technical Note—A Polynomial Algorithm for the Equal Capacity
  p-Center Problem on Trees
  ...
  */
  tuple<weight_type, vector<int>, vector<int>> k_centers(int C, int k) {
    ...
    function<void(int, int, int, int)> dfs = [&](int beg, int s, int p,
                                                 weight_type d) {
      D[beg][s] = d;

      if (beg == root) {
        if (p != -1)
          ancestors[s] = ancestors[p];
