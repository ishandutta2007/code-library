...
void GaleShapley() {
  while (!free_man.empty()) {
    ...
    int w =
        m_pref.at(m)
            .front(); // first woman on m's list to whom m has not yet proposed
    if (is_woman_free.at(w)) {
      ...
      int m_ = engaged.at(w);
      if (w_pref.at(w).at(m) > w_pref.at(w).at(m_)) { // w prefers m to m'
        free_man.push(m_);
        engaged.at(w) = m;
        free_man.pop();
      }
...
int main() {
  ...
  while (t--) {
    ...
    // sort and output
    vector<pair<int, int>> output(StableMatching::engaged.size()); // m to w
    for (int j = 1; j <= n; ++j) {
