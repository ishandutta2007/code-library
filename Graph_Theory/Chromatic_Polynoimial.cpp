...
/*The chromatic polynomial P(x), is the number of ways to color a graph in <= x
colors
trees with n nodes = x * (x - 1) ^ (n - 1)
Cycle with n nodes = (x - 1) ^ n + (-1) ^ (n) * (x-1)
For chromatic number check the smallest k such that P(k) != 0. For high
probability compute P(k) under different modulo*/
