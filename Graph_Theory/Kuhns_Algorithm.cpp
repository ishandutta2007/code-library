...
// Given a graph having perfect matching, find the number of edges which are not
// in all Maximum Matchings in a Bipartite Graph
int32_t main() {
  ...
  for (int i = 1; i <= n; i++) {
    int u = i, v = M.l[i];
    if (idx[u] == idx[v + n])
      ans++; // if nodes of a matched edge are in the same SCC then we can
             // replace it
  }
  cout << ans << '\n';
  return 0;
