...
pair<int, int> getCommonPath(int u, int a, int v,
                             int b) { // returns common path between u->a and
                                      // v->b where u is ancestor a and v is
                                      // ancestor b
  if (!isanc(a, v))
    return make_pair(0,
                     0); // v has to be an ancestor of a to have any common path
  int x = lca(a, b);
  if (dep[v] < dep[u]) { // v is an ancestor of u, so the beginning of common
                         // path should be from u
    if (isanc(x, u))     // if u is not an ancestor of x no common path exists
      return make_pair(u, x); // u->x is the common path
  } else { // u is an ancestor of v, so the beginning of common path should be
           // from v
...
pair<int, int> path_intersection(int a, int b, int c, int d) {
  if (a == 0 || b == 0)
    return {0, 0};
  int u = lca(a, b),
      v = lca(c, d); // splits both paths in two chains from lca to one node
  pair<int, int> x, y;
  ...
  while (t--) {
    ...
    while (q--) {
      ...
      if (ans.first == 0)
        ...
        cout << dep[ans.first] + dep[ans.second] -
                    2 * dep[lca(ans.first, ans.second)] + 1
             << '\n';
    }
    for (int i = 0; i <= n; i++) {
      g[i].clear();
