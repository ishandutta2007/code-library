...
struct SternBrocotTree {
  ...
  static bool validate(const Fraction &frac) {
    return validate(frac.a, frac.b);
  }

  std::array<Fraction, 2> bounds;

  // NOTE: root contains all the valid fractions, except 0/1 and 1/0.
  static SternBrocotTree get_root() {
    return SternBrocotTree(Fraction(0, 1), Fraction(1, 0));
  }

  SternBrocotTree() = default;
  SternBrocotTree(const Fraction &lbound, const Fraction &rbound)
  ...
  // Returns the maximum k after which the target is still inside the subtree of
  // current node.
  // Any reachable fraction (a, b) can be reached in O(log(max(a, b)))
  // non-trivial `approach` calls.
  long long approach(int side, const Fraction &target) const {
    long long l = 0, r = std::max(target.a, target.b);
    while (l <= r) {
      long long mid = (l + r) >> 1;
      if (this->go(side, mid).contains(target))
        l = mid + 1;
      else
        r = mid - 1;
    }
