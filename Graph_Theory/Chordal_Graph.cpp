...
/*A graph is chordal if any cycle C with |C| >= 4 has a chord i.e. an edge (u,
v) where u and v is in the cycle but (u, v) is not
A perfect elimination ordering in a graph is an ordering of the vertices of the
graph such that,
for each vertex v, v and the neighbors of v that occur after v in the order form
a clique.
...
notice that if we add a directed edge from u to v if(pos[u] < pos[v])  the we
will find a DAG.
optimal vertex coloring of the graph: col[i] = smallest color that is not used
by any of the parents of i in the DAG
chromatic number = max clique
chromatic polynomial = (x - d_1) * (x - d_2) * ... * (x - d_n) where d_i =
number of neighbors of i that come after i in PEO
*/
