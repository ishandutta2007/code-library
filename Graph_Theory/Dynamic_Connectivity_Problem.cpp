...
void dfs(int v, int value) {
  u[v] = 1, color[v] = value;
  forn(i, sz(g[v])) if (!u[g[v][i]]) dfs(g[v][i], value);
}
...
void go(int l, int r, const List &v, int vn, int add_vn) // [l, r)
{
  ...
  int vn1 = 0;
  forn(i, vn) u[i] = 0;
  forn(i, vn) if (!u[i]) deg[vn1] = 0, dfs(i, vn1++);
  ...
  vn = vn1, vn1 = 0;
  forn(i, vn) u[i] = vn1, vn1 += (deg[i] > 0), add_vn += !deg[i];
  forn(i, sz(v1)) {
    v1[i].a = u[v1[i].a];
    v1[i].b = u[v1[i].b];
  }
...
#define NAME "connect"
  ...
  forit(it, m) if (it->second != -1)
      v.pb({it->first.first, it->first.second, it->second, k});

  go(0, k, v, n, 0);

  forn(i, k) if (cnt[i + 1] != cnt[i]) printf("%d\n", ans[i]);
  return 0;
}
