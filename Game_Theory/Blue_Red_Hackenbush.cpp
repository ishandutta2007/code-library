...
/**
Chef has a rooted tree G, consisting of N vertices. Each edge of the tree has a
color black or white.

Once Chef's friend Ksen offered Chef to play a game on the tree G. The game has
very simple rule:
...
Chef really likes the game. The only problem is that Ksen knows the game very
well.
So she always plays optimally. That's why Chef has decided to choose the order
of moves.
Help Chef: tell him who will win the game if he moves first, and also who will
win the game
...
We need to take care of the big numbers efficiently. We will call these numbers
surreal numbers

since we know our answer is always of form a/2k we can store [a,k] and make
...
If a is negative and abs(a)<2k, then we have to just add 2 ie. a<-2k+1+a and k
remains same.
If a is negative and abs(a)≥2k, we add 2 to [abs(a)%2k,k]. We can quickly find
abs(a)%2k by using the fact a % 2k = a & (2k - 1)
...
// in this implementation we assume that the first person selects the 0 marked
// edges
int32_t main() {
  int t;
  cin >> t;
