...
struct ostream {
	...
	u32 map[10000];
	ostream () {
		for (int i = 0; i < 10000; ++i) map[i] = i % 10 + 48 << 24 | i / 10 % 10 + 48 << 16 | i / 100 % 10 + 48 << 8 | i / 1000 + 48;
		vout = buf + size;
	}
	~ostream () {fwrite(vout, 1, buf + size - vout, stdout);}
	inline ostream & operator << (u64 x) {
		for (; x >= 10000; x /= 10000) *--(u32*&)vout = map[x % 10000];
		do *--vout = x % 10 + 48; while (x /= 10);
		return *this;
	}
	inline ostream & operator << (char x) {return *--vout = x, *this;}
	inline ostream & operator << (const char *s) {const int L = strlen(s); return memcpy(vout -= L, s, L), *this;}
} cout;
...
int main() {
	...
	for (i = 0; i < T; ++i) cin >> a >> b, ans[i] = nimbers::product(a, b);
	for (i = T - 1; i >= 0; --i) cout << '\n' << ans[i];
	return 0;
}
