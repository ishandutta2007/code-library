...
| --|$\sum_{i\le x} \sigma_0(i) = \sum_{k \le n} \lfloor{n/k}\rfloor =$</br>$\sum_{k \le \sqrt n} \lfloor{n/k}\rfloor + \sum_{1 \le v \le \sqrt n} (\lfloor{n/v}\rfloor-\lfloor{n/(v+1)}\rfloor)$|$O(n^{1/2})$|--|--|--|--|
