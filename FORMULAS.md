...
| $f(x)=\sum_{k\le x} \omega(k)$|$\sum_{i\le x} f(x/i^2)=\sum_{Number of divisors of n that are smaller than sqrt(n)}$|$O(n^{3/4})$|$O(n^{1/2})$|--|--|--|
...
| $f(x)=\sum_{k\le x} [\Omega(k)=1]$|$\sum_{i\le x} f(x/i) = 1 + \sum_{k \le n-1} \Pmega(k)$|--|--|--|--|--|
