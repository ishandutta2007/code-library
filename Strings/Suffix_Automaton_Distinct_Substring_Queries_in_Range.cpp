...
void mark(int x, int v) { lazy[x] = last[x] = v; }
void push(int x) {
  if (lazy[x]) {
    if (c[x][0]) {
      mark(c[x][0], lazy[x]);
...
bool is_root(int x) { return c[par[x]][0] != x && c[par[x]][1] != x; }
void rotate(int x) {
...
int32_t main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    // st.query(l, l) = number of distinct substrings which lastly occured in
    // starting position l for prefix [1, i]
    for (auto[l, id] : Q[i]) {
