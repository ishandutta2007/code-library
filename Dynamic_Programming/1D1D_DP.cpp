...
The required condition is the quadrangle inequality: for all i<=j,
w(i,j)+w(i+1,j+1)<=w(i+1,j)+w(i,j+1)
From a practical point of view, we don't need to prove that w satisfies such an
inequality,
...
Let k(x) be the optimal index for dp(x) i.e. dp(x) is minimum for index k(x)
i.e. dp(x)=dp(k(x))+w(k(x),x)
Then for all i<=j, k(i)<=k(j) because of the quadrangle inequality
If we can't use CHT for this kind of DP optimization, GENERALLY they are
solvable using this trick.
Complexity: O(nlogn)*/
