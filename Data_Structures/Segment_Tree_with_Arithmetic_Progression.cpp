...
struct ST {
  struct Int { // arithmetic progression a, a + d, a + 2 * d, ...
    ...
    const Int operator+(const Int &b) const { return Int(a + b.a, d + b.d); }
  };
  mint t[N << 2];
  Int lazy[N << 2];
  ST() {
  ...
  void push(int n, int b, int e) {
    ...
    if (b != e) {
      ...
      lazy[n << 1 | 1] =
          lazy[n << 1 | 1] + lazy[n].shift(((b + e) >> 1) + 2 - b);
    }
    lazy[n] = Int(0, 0);
  }
  ...
  for (int w = 0; w < k; w++) {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      if (i * 2 <= n) {
        ...
        if (i < j)
          t.upd(1, 1, n, i, j - 1, {a[i] * i, 0});

        if (i > 1)
          t.upd(1, 1, n, 1, i - 1, {a[i], a[i]});
      } else {
        if (i < n)
          t.upd(1, 1, n, i + 1, n, {a[i] * (n - i), mint(mod) - a[i]});

        int j = n - i + 1;
        t.upd(1, 1, n, 1, j, {a[i], a[i]});
        if (j < i)
          t.upd(1, 1, n, j + 1, i, {a[i] * (n - i + 1), 0});
      }
    }
    for (int i = 1; i <= n; i++) {
