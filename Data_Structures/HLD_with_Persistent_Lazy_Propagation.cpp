...
#define rc t[cur].r
  ...
  inline int push(int pre, int b, int e, mint af = 0, mint as = 0) {
    if (ty == 2) {
      ...
      if (b == e)
        ...
        int len = e - b + 1;
        t[cur].val =
            t[cur].val + get_kth(t[cur].lf, t[cur].ls, len + 2) - t[cur].ls;
  ...
  int upd(int pre, int b, int e, int i, int j, int x) {
    ...
    if (i <= b && e <= j) {
      if (ty == 2)
        cur = push(cur, b, e, f[j - e + 1 + x], f[j - e + 2 + x]);
      else
        cur = push(cur, b, e, f[b - i + 1 + x], f[b - i + 2 + x]);
      return cur;
    }
    int mid = b + e >> 1;
...
vector<array<int, 3>> query(int u, int v) {
  ...
  if (v != l)
    res = query_up(v, kth(v, dep[v] - dep[l] - 1), 1);
...
int32_t main() {
  ...
  for (int i = 1; i <= q; i++) {
    ...
    if (ty == "A") {
      ...
      for (auto e : path) {
        if (e[2])
          p = t.upd(p, 1, n, e[0], e[1], x);
        else
          pr = r.upd(pr, 1, n, e[0], e[1], x);
        x += e[1] - e[0] + 1;
      }
    } else if (ty == "QS") {
      int u, v;
      ...
      if (u == v) {
        ...
        int x = kth(u, d - 1);
        ans = t.query(p, 1, n, 1, n) + r.query(pr, 1, n, 1, n);
        ans = ans - t.query(p, 1, n, st[x], en[x]) -
              r.query(pr, 1, n, st[x], en[x]);
      } else
        ans = t.query(p, 1, n, st[v], en[v]) + r.query(pr, 1, n, st[v], en[v]);
      cout << ans << '\n';
      last = ans.value;
    } else if (ty == "QC") {
      int u, v;
