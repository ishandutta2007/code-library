...
template <typename T> struct PersistentArray { // 0-indexed
  struct node {
    node *l, *r;
    T x;
  };
  ...
  // update the ith value if the rth array by x and return the new root of the
  // array
...
template <typename T> struct PersistentQueue {
  ...
  T pop(int r) { // pops the top element of the rth queue and returns the popped
                 // element
    st.upd(r, L[r], 0);
    L[cur] = L[r] + 1;
    R[cur] = R[r];
