...
l[i]=left child of i, i.e. nearest value <i such that a[l[i]]<a[i] (for min
heap)
r[i]=right child of i, i.e. nearest value >i such that a[r[i]]<a[i] (for min
heap)
...
// given an array find the probability of any array having each element in
// [0,1](real numbers)
// and isomorphic to the array
// two array is isomorphic if index set(before sorting) of first array is equal
// to index set(before sorting) of
...
int main() {
  ...
  while (t--) {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      a[i] = make_pair(
          x,
          -i); // for making the array elements distinct,for min heap use (+i)
    }
