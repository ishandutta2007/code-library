...
struct Block {
  ...
  int get(int x) {
    ...
    return sorted_v.size() - (lower_bound(sorted_v.begin(), sorted_v.end(), x) -
                              sorted_v.begin());
  }
  int size() { return sz; }
...
// returns such 𝑗(0 indexed), that the 𝑖-the element is the beginning of the
// 𝑗-th segment
int split(int i) {
  if (i > n)
    return blocks.size();
    ...
    if (ty[i] == 1) { // for all a_i: l <= i <= r, set a_i = x
      ...
      while (p < q) {
        ...
        ++p;
               ...
               3) { // find the number of elements in [l, r] s.t. a_i >= x
      int ans = 0;
      int p = split(l[i]), q = split(r[i] + 1);
      while (p < q) {
