...
template <typename T, T x_low, T x_high, T id> struct DynamicLiChaoTree {
  ...
  T query(const Node *t, const T &l, const T &r, const T &x) const {
    ...
    if (m == r)
      --m;
    if (x <= m)
      return max(t->x.get(x), query(t->l, l, m, x));
    else
      return max(t->x.get(x), query(t->r, m + 1, r, x));
  }
...
int main() {
  ...
  auto down_path = MFP([&](auto down_path, int idx, int par, int64 ans,
                           int64 sz, int64 all) -> int64 {
    ++sz;
    ans += A[idx] * sz;
    all += A[idx];
    bool have = false;
    ...
    if (!have)
      lct.add_line(all, ans);
    return ret;
  });

  auto up_path = MFP([&](auto up_path, int idx, int par, int64 ans, int64 sum,
                         int64 sz) -> int64 {
    sum += A[idx];
    ans += sum;
    sz++;
    ...
    if (!have)
      chmax(ret, max(lct.query(sz), 0LL) + ans);
    return ret;
  });
