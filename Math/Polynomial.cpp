...
template <int32_t MOD> struct modint {
  ...
  modint sqrt() const {
    ...
    if (pow((MOD - 1) >> 1) == MOD - 1)
      return 0; // does not exist, it should be -1, but kept as 0 for this
                // program
...
struct poly {
  ...
  vector<mint> chirpz_even(mint z,
  ...
  poly
  build(vector<poly> &ans, int v, int l, int r,
        vector<mint> &vec) { // builds evaluation tree for (x-a1)(x-a2)...(x-an)
    if (l == r)
  ...
  vector<mint> eval(vector<poly> &tree, int v, int l, int r,
                    vector<mint> &vec) { // auxiliary evaluation function
    if (l == r)
      return {eval(vec[l])};
    if (size() < 400) {
  ...
  // O(nlog^2n)
  vector<mint>
  eval(vector<mint> x) { // evaluate polynomial in (x_0, ..., x_n-1)
    int n = x.size();
    if (is_zero())
  ...
  poly interpolate(vector<poly> &tree, int v, int l, int r, int ly, int ry,
                   vector<mint> &y) { // auxiliary interpolation function
    if (l == r)
      return {y[ly] / a[0]};
    int mid = l + r >> 1;
    int midy = ly + ry >> 1;
...
poly interpolate(
    vector<mint> x,
    vector<mint> y) { // interpolates minimum polynomial from (xi, yi) pairs
  int n = x.size();
...
// resultant = A(rb_0) * ... A(rb_m). It is 0 iif there is a common root between
// A and B
// O(a.size() * b.size())
mint resultant(poly a,
               poly b) { // computes resultant of a and b, assert(!a.is_zero())
