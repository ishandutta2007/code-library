...
// number of length n sequences with elements in [0, 2^k) such that its basis
// has size r
mint count_basis(int n, int k, int r) {
  return q_binom(n, r) * q_perm(k, r) * mint(2).pow(1LL * r * (r - 1) / 2);
}

// number of length n sequences with elements in [0, 2^k) such that no subset
// has xor 0
...
// number of length n sequences with elements in [0, 2^k) such that no subset
// has xor 1 and its basis has size r
...
// number of length i sequences with elements in [0, 2^k) such that no subset
// has xor 1
// for all 1 <= i <= n
// O(n logn)
vector<mint> count_except_xor_1_for_all_n(int n, int k) {
  ...
  for (int i = 0; i <= n; i++) {
    if (i <= k - 1) {
      a[i] = (mint(2).pow(1LL * i * (i + 1) / 2) * q_inv_fac[i] *
              q_inv_fac[k - 1 - i])
                 .value;
    } else {
      a[i] = 0;
