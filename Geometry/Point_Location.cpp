...
// you are given a planar subdivision without no vertices of degree one and
// zero,
...
vector<edge *> sweepline(vector<edge *> planar, vector<PT> queries) {
  ...
  for (int x = 0; x < cid; x++) {
    ...
    for (Event event : events[x]) {
      ...
      if (event.type == GET) {
        auto jt =
            vert.upper_bound(make_pair(queries[event.pos].y, planar.size()));
        if (jt != vert.begin()) {
          --jt;
          int i = jt->second;
...
int32_t main() {
  ...
  for (int i = 0; i < n; i++) {
    if (ret[i].first) {
      ...
      ans[planar.body[ret[i].second]->id]++;
  }
  for (int i = 0; i < q; i++)
    cout << ans[i] << '\n';
  return 0;
