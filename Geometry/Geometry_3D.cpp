...
double get_angle(line3d l1, line3d l2) { return small_angle(l1.d, l2.d); }
bool is_parallel(line3d l1, line3d l2) { return l1.d * l2.d == zero; }
bool is_perpendicular(line3d l1, line3d l2) { return sign((l1.d | l2.d)) == 0; }
double get_angle(plane p1, plane p2) { return small_angle(p1.n, p2.n); }
bool is_parallel(plane p1, plane p2) { return p1.n * p2.n == zero; }
bool is_perpendicular(plane p1, plane p2) { return sign((p1.n | p2.n)) == 0; }
double get_angle(plane p, line3d l) { return PI / 2 - small_angle(p.n, l.d); }
bool is_parallel(plane p, line3d l) { return sign((p.n | l.d)) == 0; }
...
pair<p3, p3> plane_plane_intersection(p3 a1, p3 b1, p3 c1, p3 a2, p3 b2,
                                      p3 c2) {
  p3 n1 = (b1 - a1) * (c1 - a1);
  p3 n2 = (b2 - a2) * (c2 - a2);
...
bool operator<(p3 p, p3 q) { return tie(p.x, p.y, p.z) < tie(q.x, q.y, q.z); }
struct edge {
  int v;
...
struct CH3D {
  ...
  double len(p3 a) { return sqrt(a | a); }
  p3 cross(const p3 &a, const p3 &b, const p3 &c) { return (b - a) * (c - a); }
  double area(p3 a, p3 b, p3 c) { return len((b - a) * (c - a)); }
  double volume(p3 a, p3 b, p3 c, p3 d) { return (b - a) * (c - a) | (d - a); }
...
bool proper_intersection(p3 a, p3 b, p3 c, p3 d, p3 &out) {
  ...
  int oa = sign(cd | a), ob = sign(cd | b), oc = sign(ab | c),
      od = sign(ab | d);
...
Set segment_segment_intersection_on_sphere(p3 a, p3 b, p3 c, p3 d) {
  ...
  p3 out;
  if (proper_intersection(a, b, c, d, out))
    return {out};
  Set s;
  if (point_on_sphere_segment(c, d, a))
    s.insert(a);
  if (point_on_sphere_segment(c, d, b))
    s.insert(b);
  if (point_on_sphere_segment(a, b, c))
    s.insert(c);
  if (point_on_sphere_segment(a, b, d))
    s.insert(d);
  return s;
}

double angle_on_sphere(p3 a, p3 b, p3 c) { return get_angle(a * b, a * c); }
...
double oriented_angle_on_sphere(p3 a, p3 b, p3 c) {
  if ((a * b | c) >= 0)
    ...
    return 2 * PI - angle_on_sphere(a, b, c);
}
...
// 1 if O is inside the polyhedron, and the vector areas of the faces are
