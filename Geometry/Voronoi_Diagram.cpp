...
// 'circ'.
// returns a vector v where v[i], v[i + 1], v[i + 2] form a triangulation in ccw
// order
// such that i % 3 == 0
...
bool circ(P p, P a, P b, P c) { // is p in the circumcircle?
  i128 p2 = p.norm2(), A = a.norm2() - p2, B = b.norm2() - p2,
       C = c.norm2() - p2;
...
PT circle(PT a, PT b, PT c) { // radius of the circumcircle
  b = (a + b) * 0.5;
  c = (a + c) * 0.5;
  auto p = line_line_intersection(b, b + rotatecw90(a - b), c,
                                  c + rotatecw90(a - c));
  return p;
}
...
double ori(PT a, PT b, PT c) { return cross(b - a, c - a); }
...
inline void yo(PT a, vector<PT> &can) {
  for (int i = 0; i < p.size(); i++) {
    ans = max(ans,
              compute(project_from_point_to_seg(p[i], p[(i + 1) % p.size()], a),
                      can));
  }
}
int32_t main() {
