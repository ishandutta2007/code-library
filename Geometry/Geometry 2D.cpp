...
// 1 if point is ccw to the line, 2 if point is cw to the line, 3 if point is on
// the line
...
// intersection between segament ab and line cd assuming unique intersection
// exists
...
double dist_from_seg_to_seg(PT a, PT b, PT c, PT d) {
  ...
  if (seg_seg_intersection(a, b, c, d, dummy))
    ...
    return min(
        {dist_from_point_to_seg(a, b, c), dist_from_point_to_seg(a, b, d),
...
int get_circle(line u, PT q, double r1, circle &c1, circle &c2) {
  ...
  if (!v.size())
    v = circle_line_intersection(q, r1, u2.a, u2.b);
  v.push_back(v[0]);
  p1 = v[0], p2 = v[1];
  c1 = circle(p1, r1);
  if (p1 == p2) {
...
int tangents_lines_from_circle(PT c1, double r1, PT c2, double r2, bool inner,
                               line &u, line &v) {
  if (inner)
    r2 = -r2;
  PT d = c2 - c1;
  ...
  if (out.size() == 2)
    v = line(out[1].first, out[1].second);
  return 1 + (h2 > 0);
}
// O(n^2 log n)
struct CircleUnion {
...
double dist_from_point_to_polygon(vector<PT> &v, PT p) { // O(log n)
  ...
  if (n <= 3) {
    double ans = inf;
    for (int i = 0; i < n; i++)
      ans = min(ans, dist_from_point_to_seg(v[i], v[(i + 1) % n], p));
    return ans;
  }
...
double dist_from_polygon_to_line(vector<PT> &p, PT a, PT b,
                                 int top) { // O(log n)
  PT orth = (b - a).perp();
  if (orientation(a, b, p[0]) > 0)
    orth = (a - b).perp();
  int id = extreme_vertex(p, orth, top);
  if (dot(p[id] - a, orth) > 0)
    return 0.0; // if orth and a are in the same half of the line, then poly and
                // line intersects
...
pair<PT, int> point_poly_tangent(vector<PT> &p, PT Q, int dir, int l, int r) {
  while (r - l > 1) {
    ...
    if (!pvs) {
      if (orientation(Q, p[mid], p[l]) == dir)
        r = mid - 1;
  ...
  for (int i = l + 1; i <= r; i++)
    ret = orientation(Q, ret.first, p[i]) != dir ? make_pair(p[i], i) : ret;
  return ret;
}
...
vector<PT> minkowski_sum(vector<PT> &a, vector<PT> &b) {
  ...
  int i = 0, j = 0; // assuming a[i] and b[j] both are (left, bottom)-most
                    // points
  vector<PT> c;
  c.push_back(a[i] + b[j]);
  while (i + 1 < n || j + 1 < m) {
...
double polygon_circle_intersection(vector<PT> &v, PT p, double r) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (x == 0)
      continue;
    double area =
        triangle_circle_intersection(org, r, v[i] - p, v[(i + 1) % n] - p);
    if (x < 0)
      ans -= area;
    else
      ans += area;
