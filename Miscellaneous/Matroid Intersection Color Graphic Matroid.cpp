...
struct GraphicOracle {
  struct GraphBasis {
    ...
    bool independent_with(T edge) { return !same(edge.first, edge.second); }
  };
  int n;
  ...
  bool can_insert(int id) { return basis.independent_with(elements[id].edge); }
...
// (c1_prep + c2_prep))
vector<int> matriod_intersection(int node_count,
                                 vector<GroundSetElement> _elements) {
  elements = _elements;
  independent_set.clear();
  int color_count = 0;
...
int32_t main() {
  ...
  while (cin >> n >> m >> k) {
    ...
    //   cout << elements[i].edge.first << ' ' << elements[i].edge.second << ' '
    //   << elements[i].color << '\n';
    // }
    cout << "Instancia " << ++cs << "\n";
